...
void DriverCopy(Season season,int place1, int place2)
{
    Driver tempDriver1 = season->drivers[place1];
    season->drivers[place1] = season->drivers[place2];
    season->drivers[place2] = tempDriver1;
}

void TeamCopy(Season season,int place1, int place2)
{
    Team tempTeam1 = season->teams[place1];
    season->teams[place1] = season->teams[place2];
    season->teams[place2] = tempTeam1;
}
    ...
    for (int i = 0; i < (objectNumber - 1); i++)
    {
        for (int j = 0; j < (objectNumber - i - 1); j++)
        {
            if ((points[j] < points[j + 1]) || (((points[j] == points[j + 1]) && desicionMaker[j] > desicionMaker[j + 1])))
            {
                if (type != 'D')
                {
                    TeamCopy(season, j, j+1);
                }
                else
                {
                    swap(&desicionMaker[j], &desicionMaker[j + 1]);
                    DriverCopy(season, j, j+1);
                }
